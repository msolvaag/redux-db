...
export default class ExtendedTableModel<R extends ReduxDB.TableRecord>
    extends ReduxDB.TableModel<R> implements ExtendedTable<R> {
    ...
    getByFk(fieldName: string, id: number | string): ReduxDB.TableRecordSet<R> {
        ...
        const pk = ReduxDB.utils.ensureID(id);
    ...
    getFieldValue(id: string | number, field: keyof ReduxDB.ValueType<R>) {
