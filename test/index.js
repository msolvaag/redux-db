...
test('insert records', function (t) {
    ...
    const [recordModel] = BlogPost.insert(data.blogPosts);
...
test('inserting data without PK. Using the onGeneratePK callback.', function (t) {
    t.plan(4);
    ...
    let seq = 0;
    db.options.onGeneratePK = (record, schema) => {
    ...
    t.assert(post1.id === "injected_0", "Missing PK's is generated by the callback.");
    t.assert(post2.id === "injected_1", "Missing PK's is generated by the callback.");
...
test('updating data without PK.', function (t) {
    ...
    }, "Updating a record without a valid ID fails even if the onGeneratePK callback is defined.");

    t.assert(seq === 0, "The onGeneratePK callback is never called during updates.");
});
